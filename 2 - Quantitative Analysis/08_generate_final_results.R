...
#   - Update `sap_dir` so it points to the local folder where the SAP
#     dataset-builder outputs are stored.
#   - `etable_dir` is created automatically inside `sap_dir` and stores the final
#     PAP/SAP results, multiple-testing tables, model diagnostics, and RDS model
#     objects produced by this script.
#
# Example:
#   output_root <- "C:/Users/YourName/Drive/IADB_outputs"
#
#   sap_dir <- file.path(output_root, "data", "clean", "sap_dataset_builder")
...
# ------------------------------------------------------------------------------
# Harmonizibg modeling variables -----------------------------------------------
# ------------------------------------------------------------------------------
...
#   M3_country_fe:       M2 + country fixed effects
...
# ------------------------------------------------------------------------------
# Defining PAP-primary and sensitivity samples ---------------------------------
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# Estimating PAP/SAP model families --------------------------------------------
# ------------------------------------------------------------------------------
# Main PAP-primary outcomes ----------------------------------------------------
...
# Sensitivity/sample-robustness outcomes -------------------------------------
...
# 8Family 1: channel comparisons, Romano-Wolf stepdown ------------------------
...
run_romano_wolf_channel_family <- function(
  ...
  estimate_lm_model <- function(spec) {
    df_model <- prep_model_data(spec$data, spec$outcome) |>
      mutate(
        # Ensuring the FE and cluster variable is present in the model frame.
  ...
  run_one_boot_test <- function(outcome_label, term) {
    # Setting both RNGs. fwildclusterboot uses dqrng for common weight types.
    set.seed(seed)
...
# Family 2: amount and delivery, Holm-Bonferroni -----------------------------
...
# ------------------------------------------------------------------------------
# Sample-size and skipped-model diagnostics ------------------------------------
# ------------------------------------------------------------------------------
...
# Optional: LaTeX tables using modelsummary ------------------------------------
